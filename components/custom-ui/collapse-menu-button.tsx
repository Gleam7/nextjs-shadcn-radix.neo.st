...
export function CollapseMenuButton({ icon, label, submenus, isOpen }: CollapseMenuButtonProps) {
	...
	return isOpen ? (
		<Collapsible open={isCollapsed} onOpenChange={setIsCollapsed} className="w-full">
			<CollapsibleTrigger className="[&[data-state=open]>div>div>svg]:rotate-180 mb-1" asChild>
				<Button variant={isSubmenuActive ? 'secondary' : 'ghost'} className="w-full px-0 justify-start h-10">
			...
			<CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
				{submenus.map(({ href, label, active }, index) => (
					<Button
						...
						className="w-full justify-start px-2 h-10 mb-1"
						asChild
					>
						<Link href={href}>
							<span className="">
				...
				<Tooltip delayDuration={100}>
					<TooltipTrigger asChild>
						<DropdownMenuTrigger asChild>
							<Button variant={isSubmenuActive ? 'secondary' : 'ghost'} className="w-full justify-start h-10 mb-1 px-0">
								<div className="w-full items-center flex justify-between">
									<div className="flex items-center">
			...
			<DropdownMenuContent side="right" sideOffset={25} align="start">
				...
				{submenus.map(({ href, label, active }, index) => (
					<DropdownMenuItem key={index} asChild>
						<Link className={`cursor-pointer ${((active === undefined && pathname === href) || active) && 'bg-secondary'}`} href={href}>
